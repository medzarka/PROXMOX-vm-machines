...
PASS_TARGET="vms"
COMMON_DATA=("default_domain"  "default_user"  "default_password_length")
COMMON_AUTOGENERATED_DATA=()
STORAGE=local-lvm  
...
load_vm_data(){
    ...
    DATA=$1
    AUTOGENERATED_DATA=$2

    sudo mkdir -p .env
    for A_DATA in "${DATA[@]}"
    do
        echo "####### Load the DATA: $A_DATA"
        DATA_PATH="pve01/vm/common/$A_DATA"
    ...
    for A_DATA in "${AUTOGENERATED_DATA[@]}"
    do
        echo "####### Load the DATA: $A_DATA"
        DATA_PATH="pve01/vm/common/$A_DATA"
        DATA_VALUE=$(sudo pass $DATA_PATH)  
        if [[ -z "$DATA_VALUE" ]]; then 
            echo "The $A_DATA variable is not defined, it will be generated."
...
create_retrive_common_autogenerated_variable_from_pass()
{
    ...
    if [ -n "$DATA_NAME" ] && [ -n "$DEFAULT_PASSWORD_LENGTH" ]; then 
        echo "####### Load the AUTOGENERATED DATA: $DATA_NAME"
        DATA_PATH="pve01/$PASS_TARGET/common/$DATA_NAME"
        DATA_VALUE=$(sudo pass $DATA_PATH)  
        if [[ -z "$DATA_VALUE" ]]; then 
