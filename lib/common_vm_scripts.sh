...
load_vm_data(){
    ...
    do
        if [ "$A_DATA" != "" ]
        then
            echo "####### Load the VM DATA: $A_DATA"
            DATA_PATH="pve01/$PASS_TARGET/$VMID/$A_DATA"
            DATA_VALUE=$(sudo pass $DATA_PATH)
            if [[ -z "$DATA_VALUE" ]]; then
                echo "The $A_DATA variable is not defined, please type it twice (like a password):"
                sudo pass insert $DATA_PATH
                DATA_VALUE=$(sudo pass $DATA_PATH)  
                echo "The $A_DATA is defined to $DATA_VALUE"
            else
                echo "The $A_DATA is already defined to $DATA_VALUE"
            fi
            echo "$DATA_VALUE" | sudo tee -a ".env/secret_$A_DATA" > /dev/null
        fi
    done

    for AA_DATA in "${AUTOGENERATED_DATA[@]}"
    do
        if [ "$AA_DATA" != "" ]
            then
            echo "####### Load the VM AUTOGENERATED DATA: $AA_DATA"
            DATA_PATH="pve01/$PASS_TARGET/$VMID/$AA_DATA"
            DATA_VALUE=$(sudo pass $DATA_PATH)  
            if [[ -z "$DATA_VALUE" ]]; then 
                echo "The $A_DAA_DATAATA variable is not defined, it will be generated."
                sudo pass generate $DATA_PATH $DEFAULT_PASSWORD_LENGTH -n 
                DATA_VALUE=$(sudo pass $DATA_PATH)  
                echo "The $AA_DATA is defined to $DATA_VALUE"
            else
                echo "The $AA_DATA is already defined to $DATA_VALUE"
            fi
...
create_retrive_common_variable_from_pass()
{
    ...
    if [ -n "$DATA_NAME" ]; then 
        ...
        DATA_PATH="pve01/$PASS_TARGET/common/$DATA_NAME"
        DATA_VALUE=$(sudo pass $DATA_PATH)  
        if [[ -z "$DATA_VALUE" ]]; then 
...
create_retrive_vm_variable_from_pass()
{
    ...
    if [ -n "$VMID" ] && [ -n "$DATA_NAME" ]; then 
        ...
        DATA_PATH="pve01/$PASS_TARGET/$VMID/$DATA_NAME"
        DATA_VALUE=$(sudo pass $DATA_PATH)  
        if [[ -z "$DATA_VALUE" ]]; then 
            echo "The $DATA_NAME variable is not defined, please type it twice (like a password):"
            sudo pass insert $DATA_PATH
